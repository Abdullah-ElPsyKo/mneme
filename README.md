...
**A local home for your memory.**

Mneme is a local-first personal memory system designed to preserve more than notes. It stores knowledge, decisions, preferences, projects, events, procedures, structured facts, relationships, evidence, and how that information changes over time.

Rather than treating an AI model as the memory itself, Mneme keeps the underlying memory user-owned and persistent. AI is an optional interface for recalling and reasoning over that information.

## Development status

Mneme is currently an experimental personal project under active development.

The initial implementation was built extensively with AI-assisted coding, with large portions generated by coding agents.

The current focus is no longer rapid feature generation. I actively use Mneme myself, and development is now driven primarily by problems, bugs, and friction discovered through real-world use.

Expect bugs, rough edges, architectural changes, and quality-of-life improvements while this process continues.

The program isn't perfect, but usefulness comes first. :)

## Can I explain every line of code or architectural decision?

Absolutely not.

Mneme was built because I needed it for a specific use case, not because I wanted to spend six months designing the perfect architecture before having something useful.

A large part of the initial implementation was generated with coding agents. I've studied the architecture enough to understand the system at a useful level, but I'm not going to pretend that I personally designed or can justify every abstraction, implementation detail, or architectural decision in the codebase.

What matters to me right now is that Mneme is useful, reliable with my data, and keeps improving through actual use.

If it solves a problem for you too, you're more than welcome to use it, inspect it, break it, or contribute improvements.

## Why Mneme?

Most personal knowledge tools are fundamentally document or note systems. Mneme explores a different model: treating personal information as memory with structure, history and provenance.

A memory can represent a project, decision, procedure, goal, policy, idea, note, document or quick capture. Memories can carry structured facts, belong to projects, reference entities and relationships, change over time, supersede older information, and retain their provenance.

Detailed documents and imported files can remain evidence rather than being manually converted into hundreds of notes.

The intended workflow is simple:

**Capture quickly → process when convenient → retrieve when needed.**

Mneme should reduce the effort required to remember and organize information, not become another system that needs constant maintenance.

## AI is optional

Mneme does not depend on an AI model as its source of truth.

The underlying memory remains available through local storage, structured data, search, relationships and history. Optional local or external models can provide a conversational interface over selected evidence, but models do not own the memory and should not have unrestricted authority to rewrite it.

This separation is intentional: models can change; your memory should not have to.
