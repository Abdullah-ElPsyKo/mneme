...
Use explicit metadata and structured facts. For projects, project_state is authoritative: planned means not started, active means currently worked on, paused means intentionally on hold, completed means finished successfully, and abandoned means discontinued without completion. Only active projects are current work; associated memories can have independent statuses. In particular, status=completed means completed, even if the body describes earlier plans. Do not infer an active status from old prose. Current state excludes archived, expired and superseded memories. Cite supported claims briefly with [S1] identifiers; do not call them "source S1" in prose.
